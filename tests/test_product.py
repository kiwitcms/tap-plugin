...
class Given_TCMS_PRODUCT_IsPresent(PluginTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.plugin._rpc.Product.filter = MagicMock(return_value=[{'id': 44}])
...
class GivenProductExistsInDatabase(PluginTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.plugin._rpc.TestPlan.filter = MagicMock(return_value=[])
        cls.plugin._rpc.Product.filter = MagicMock(return_value=[{'id': 44}])
        cls.plugin._rpc.Product.create = MagicMock()
    ...
    def test_when_adding_product_then_will_reuse_it(self):
        with patch.dict(os.environ, {
            ...
            self.assertEqual(product_id, 44)
            self.assertEqual(product_name, 'p.Test')
            self.plugin._rpc.Product.create.assert_not_called()
...
class GivenProductDoesntExistInDatabase(PluginTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.plugin._rpc = MagicMock()
        cls.plugin._rpc.TestPlan.filter = MagicMock(return_value=[])
        cls.plugin._rpc.Product.filter = MagicMock(return_value=[])
        cls.plugin._rpc.Classification.filter = MagicMock(
            return_value=[{'id': 4}])
        cls.plugin._rpc.Product.create = MagicMock(return_value={'id': 55})
    ...
    def test_when_adding_product_then_will_add_it(self):
        with patch.dict(os.environ, {
            ...
            self.assertEqual(product_name, 'p.Test')
            self.plugin._rpc.Product.create.assert_called_with({
                'name': 'p.Test',
                'classification_id': 4,
            })
